...
void RC2::mix(uint8_t j, uint8_t i, uint8_t s){
    R[i] += K[(j << 2) + i] + (R[(i + 3) & 3] & R[(i + 2) & 3]) + ((~R[(i + 3) & 3]) & R[(i + 1) & 3]);
    R[i] = ROL(R[i], s, 16);
}

void RC2::mash(uint8_t i){
    R[i] += K[R[(i + 3) & 3] & 63];
}

void RC2::r_mix(uint8_t j, uint8_t i, uint8_t s){
    R[i] = ROR(R[i], s, 16);
    R[i] = (R[i] - K[(j < 2) + i] - (R[(i+3) & 3] & R[(i + 2) & 3]) - ((~R[(i + 3) & 3]) & R[(i + 1) & 3]) + 65536) & 65535;
}

void RC2::r_mash(uint8_t i){
...
RC2::RC2(std::string & KEY, uint32_t t1){
    T1 = t1;
    keyset = false;
    setkey(KEY);
}

void RC2::setkey(std::string KEY){
    if (keyset){
        error(2);
    }
...
std::string RC2::encrypt(std::string DATA){
    if (!keyset){
        error(1);
    }
...
std::string RC2::decrypt(std::string DATA){
    if (!keyset){
        error(1);
    }
    for(uint8_t x = 0; x < 4; x++){
