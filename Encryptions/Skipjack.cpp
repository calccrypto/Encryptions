...
uint16_t Skipjack::G_e(uint16_t quarter, uint32_t fourkeys){
...
void Skipjack::A(uint16_t & w1, uint16_t & w2, uint16_t & w3, uint16_t & w4, uint16_t i){
    uint16_t w5 = w4;
    w4 = w3;
    w3 = w2;
...
void Skipjack::B(uint16_t & w1, uint16_t & w2, uint16_t & w3, uint16_t & w4, uint16_t i){
    uint16_t w5 = w4;
    w4 = w3;
    w3 = w1 ^ w2 ^ i;
    w2 = G_e(w1, toint(key.substr((i << 3) - 8, 8), 16));
...
uint16_t Skipjack::G_d(uint16_t quarter, uint32_t fourkeys){
    uint16_t left = quarter >> 8;
    uint16_t right = quarter & 255;
    uint8_t k0 = fourkeys >> 24;
    uint8_t k1 = (fourkeys >> 16) & 255;
...
void Skipjack::invA(uint16_t & w1, uint16_t & w2, uint16_t & w3, uint16_t & w4, uint16_t i){
    uint16_t w5 = w1 ^ w2 ^ i;
...
void Skipjack::invB(uint16_t & w1, uint16_t & w2, uint16_t & w3, uint16_t & w4, uint16_t i){
    uint16_t w5 = w1;
    w1 = G_d(w2, toint(key.substr((i << 3) - 8, 8), 16));
    w2 = w1 ^ w3 ^ i;
    w3 = w4;
...
Skipjack::Skipjack(std::string KEY){
    keyset = false;
    setkey(KEY);
}

void Skipjack::setkey(std::string KEY){
    if (keyset){
        error(2);
    }
    KEY = hexlify(KEY.substr(0, std::min((int) KEY.size(), 10)));
    while (KEY.size() < 256){
        KEY = KEY + KEY;
    }
...
std::string Skipjack::encrypt(std::string DATA){
    if (!keyset){
        error(1);
    }
...
std::string Skipjack::decrypt(std::string DATA){
    if (!keyset){
        error(1);
    }
    uint16_t w1 = toint(DATA.substr(0, 2), 256);
